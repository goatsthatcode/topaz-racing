...
// Viewport adaptation
// Wide (≥640): top-nav <details> gets open so the nav is always visible.
// Narrow (<640): top-nav is closed so the hamburger starts collapsed;
//   search and more-menu drawers open.
// The visualViewport resize listener is registered exactly once to prevent
// the original bug where stacked listeners kept re-firing on every resize.
let _viewportResizeAttached = false;
function adaptViewport() {
    ...
    if (window.innerWidth < 640) {
        ...
        getElement('top-nav')?.removeAttribute('open');
    } else {
        getElement('top-nav')?.setAttribute('open', 'open');
        if (!_viewportResizeAttached) {
            _viewportResizeAttached = true;
